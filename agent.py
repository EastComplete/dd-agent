    ...
    def reload_configs(self, checks_to_reload=set()):
        ...
        if not checks_to_reload:
            ...
            for check in checks_to_reload:
                try:
                    idx = new_checksd['initialized_checks'].index(check)
                except ValueError:
                    ...
                    if check.name in new_checksd['init_failed_checks']:
                ...
                if isinstance(fresh_check, list):
                    if idx is not None:
                        new_checksd['initialized_checks'][idx] = fresh_check
                    else:
                        new_checksd['initialized_checks'].append(fresh_check)
